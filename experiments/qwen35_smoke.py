"""Smoke test: Qwen3.5-27B (bf16) on the GB10 Spark.
...
print(f"[mem] allocated {torch.cuda.memory_allocated()/1e9:.1f}GB  reserved {torch.cuda.memory_reserved()/1e9:.1f}GB")
...
print(f"[cfg] layers={n_layers} hidden={h_size} arch={cfg.architectures}")
...
enc = tok.apply_chat_template(msgs, add_generation_prompt=True, return_tensors="pt", return_dict=True).to("cuda:0")

with torch.no_grad():
    out = model(**enc, output_hidden_states=True)
hs = out.hidden_states
mid = len(hs) // 2
print(f"[hidden_states] tuple_len={len(hs)} (embed + {len(hs)-1} layers)  per-layer shape={tuple(hs[mid].shape)}")
last = hs[mid][0, -1, :]
print(f"[act] mid-layer last-token vec: dim={last.shape[0]} norm={last.float().norm().item():.2f}")
...
print("[decode]", tok.decode(gen[0, enc["input_ids"].shape[1]:], skip_special_tokens=True)[:200].replace("\n", " "))
print("OK")
