...
def get_args():
    parser = argparse.ArgumentParser(
        description="Sequential multi-layer latent pipeline with on-the-fly ReLU-gated SVM projection."
    )
    ...
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size for delta computation and generation.")
...
def compute_additive_deltas(
    *,
    ...
    if missing:
        raise RuntimeError(f"Failed to capture deltas for layers: {missing}")
    return deltas


def register_add_hooks(layers, selected_layers: List[int], deltas: Dict[int, torch.Tensor], local_idx: Optional[int] = None):
    handles = []
    for layer_idx in selected_layers:
        delta = deltas[layer_idx]
        if local_idx is not None:
            delta = delta[local_idx]
        handles.append(layers[layer_idx].register_forward_hook(add_hook(delta)))
    return handles


def generate_with_additive_deltas(
    *,
    ...
    generation_handles = register_add_hooks(layers, selected_layers, deltas)
    try:
        ...
        for local_idx, prompt in enumerate(batch_prompts):
            single_handles = register_add_hooks(layers, selected_layers, deltas, local_idx=local_idx)
            try:
...
def main():
    ...
    base_out_dir = args.out_dir if args.out_dir is not None else os.path.join("./completions", args.model_name, "pipeline")
    ...
    results = []
    pbar = tqdm(total=len(prompts), desc="Pipeline one-step delta + Generate")
    for batch_start, batch_prompts in chunked(prompts, args.batch_size):
        ...
        deltas = compute_additive_deltas(
            inputs=inputs,
            layers=layers,
            selected_layers=selected_layers,
            model=model,
        ...
        responses = generate_with_additive_deltas(
